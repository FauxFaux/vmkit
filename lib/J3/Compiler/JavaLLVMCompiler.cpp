...
Function* JavaLLVMCompiler::parseFunction(JavaMethod* meth) {
  ...
  if (func->getLinkage() == GlobalValue::ExternalWeakLinkage) {
    ...
    if (isNative(meth->access)) {
      jit.nativeCompile();
      vmkit::MvmModule::runPasses(func, JavaNativeFunctionPasses);
    } else {
      jit.javaCompile();
      vmkit::MvmModule::runPasses(func, JavaFunctionPasses);
    }
...
void JavaLLVMCompiler::addJavaPasses() {
  ...
  vmkit::MvmModule::addCommandLinePasses(JavaFunctionPasses);
