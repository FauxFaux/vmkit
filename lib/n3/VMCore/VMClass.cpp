    ...
    if (status == ready) {
      ...
      if (meth) {
        ...
        clinit_t res = (clinit_t)
          (intptr_t)vmkit::VMKitModule::executionEngine->getPointerToGlobal(pred);
        res();
      }
...
void VMClass::resolveStaticFields(VMGenericMethod* genMethod) {
  ...
  uint64 size = vmkit::VMKitModule::getTypeSize(cl->staticType->getContainedType(0));
	...
	if (cl->isArray) {
      ...
      for (std::vector<VMMethod*>::iterator i = virtualMethods.begin(), 
        ...
        (*i)->offsetInVt = cl->vtSize++;
			...
			uint64 size = vmkit::VMKitModule::getTypeSize(cl->virtualType->getContainedType(0));
...
VMObject* VMClass::initialiseObject(VMObject* obj) {
  uint64 size = vmkit::VMKitModule::getTypeSize(virtualType->getContainedType(0));
...
VMObject* VMClassArray::doNew(uint32 nb) {
  if (status < inClinit) resolveType(true, true, NULL);
  uint64 size = vmkit::VMKitModule::getTypeSize(baseClass->naturalType);
...
VMMethod *VMMethod::compileToNative(VMGenericMethod* genMethod) {
	if(!code) {
		...
		void* res = 
			vmkit::VMKitModule::executionEngine->getPointerToGlobalIfAvailable(methPtr);
		if (res == 0) {
			classDef->aquire();
			res = 
				vmkit::VMKitModule::executionEngine->getPointerToGlobalIfAvailable(methPtr);
			if (res == 0) {
				...
				void* res = vmkit::VMKitModule::executionEngine->getPointerToGlobal(methPtr);
				code = res;
