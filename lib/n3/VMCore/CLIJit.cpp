...
N3VirtualTable* CLIJit::makeArrayVT(VMClassArray* cl) {
  ...
  void* tracer = vmkit::VMKitModule::executionEngine->getPointerToGlobal(func);
...
void CLIJit::invokeNew(uint32 value, VMGenericClass* genClass, VMGenericMethod* genMethod) {
  ...
  if (type->isPointer) {
    ...
    uint64 size = vmkit::VMKitModule::getTypeSize(type->naturalType);
...
void CLIJit::setVirtualField(uint32 value, bool isVolatile, VMGenericClass* genClass, VMGenericMethod* genMethod) {
  ...
  if (field->signature->super == MSCorlib::pValue &&
    ...
    uint64 size = vmkit::VMKitModule::getTypeSize(field->signature->naturalType);
  ...
  */
  ...
  if (localVarSig) {
    ...
    for (std::vector<VMCommonClass*>::iterator i = temp.begin(), 
      ...
      if (cl->naturalType->isSingleValueType()) {
        ...
        uint64 size = vmkit::VMKitModule::getTypeSize(cl->naturalType);
  ...
  if (PI == PE) {
    ...
    if (endType != Type::getVoidTy(getGlobalContext())) {
      ...
      const Type* lastType = 
        funcType->getContainedType(funcType->getNumContainedTypes() - 1);
      uint64 size = vmkit::VMKitModule::getTypeSize(lastType->getContainedType(0));
  ...
  vmkit::VMKitModule::runPasses(llvmFunction, VMThread::get()->perFunctionPasses);
...
void VMField::initField(VMObject* obj) {
  ...
  const TargetData* targetData = vmkit::VMKitModule::executionEngine->getTargetData();
...
void CLIJit::initialiseAppDomain(N3* vm) {
  vmkit::VMKitModule::protectEngine.lock();
  vmkit::VMKitModule::executionEngine->addModule(vm->getLLVMModule());
  vmkit::VMKitModule::protectEngine.unlock();
}
...
void CLIJit::initialiseBootstrapVM(N3* vm) {
  ...
  vmkit::VMKitModule::protectEngine.lock();
  vmkit::VMKitModule::executionEngine->addModule(module);
  vmkit::VMKitModule::protectEngine.unlock();
...
void AddStandardCompilePasses(FunctionPassManager *PM) {
  llvm::MutexGuard locked(vmkit::VMKitModule::executionEngine->lock);
