...
void VMKitJITMethodInfo::print(void* ip, void* addr) {
  fprintf(stderr, "; %p (%p) in %s LLVM method\n", ip, addr,
      ((llvm::Function*)MetaInfo)->getName().data());
}

class VMKitJITListener : public llvm::JITEventListener {
public:
  virtual void NotifyFunctionEmitted(const Function &F,
                                     void *Code, size_t Size,
                                     const EmittedFunctionDetails &Details) {
    assert(F.getParent() == VMKitModule::globalModule);
    assert(F.hasGC());
    // We know the last GC info is for this method.
    GCStrategy::iterator I = vmkit::VMKitModule::TheGCStrategy->end();
    I--;
    ...
    JITMethodInfo* MI = new(*VMKitModule::Allocator, "VMKitJITMethodInfo")
        VMKitJITMethodInfo(GFI, &F, VMKitModule::executionEngine);
    MI->addToVMKit(vmkit::Thread::get()->vmkit, (JIT*)VMKitModule::executionEngine);
  }
};
...
static VMKitJITListener JITListener;

void VMKitModule::loadBytecodeFile(const std::string& str) {
  SMDiagnostic Err;
...
void VMKitModule::initialise(CodeGenOpt::Level level, Module* M,
                           TargetMachine* T) {
  vmkit::linkVmkitGC();
...
BaseIntrinsics::BaseIntrinsics(llvm::Module* module) {
  ...
  module->setDataLayout(VMKitModule::globalModule->getDataLayout());
  module->setTargetTriple(VMKitModule::globalModule->getTargetTriple());
...
const llvm::TargetData* VMKitModule::TheTargetData;
llvm::GCStrategy* VMKitModule::TheGCStrategy;
llvm::Module *VMKitModule::globalModule;
llvm::ExecutionEngine* VMKitModule::executionEngine;
vmkit::LockRecursive VMKitModule::protectEngine;
vmkit::BumpPtrAllocator* VMKitModule::Allocator;
//unsigned VMKitModule::MetadataTypeKind;

uint64 VMKitModule::getTypeSize(const llvm::Type* type) {
...
void VMKitModule::addCommandLinePasses(FunctionPassManager* PM) {
  addPass(PM, new TargetData(*VMKitModule::TheTargetData));
